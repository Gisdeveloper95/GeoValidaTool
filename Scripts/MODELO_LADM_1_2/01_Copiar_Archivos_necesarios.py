...
def process_template_gdb(root_path, gpkg_name=None):
    template_zip = root_path / 'Files' / 'Templates' / 'MODELO_LADM_1_2' / 'GDB' / 'Template_1_2.gdb.zip'
    temp_dir = root_path / 'Files' / 'Temporary_Files' / 'MODELO_LADM_1_2'
    ...
    old_gdb = temp_dir / 'Template_1_2.gdb'
...
def analyze_and_validate_gpkg(gpkg_path, gpkg_type):
    try:
        ...
        json_filename = f"gpkg_reference_interno_1_2_{gpkg_type}.json"
        reference_json = root / "Scripts" / "MODELO_LADM_1_2" / "reference" / json_filename
...
def main():
    print("EJECUTANDO MODELO LADM COL 1.2")
    try:
        app = QApplication.instance()
        if not app:
        ...
        root = get_project_root()
        temp_files = root / 'Files' / 'Temporary_Files' / 'MODELO_LADM_1_2'
        temp_files_config = root / 'Files' / 'Temporary_Files'
        json_path = temp_files_config / 'Ruta_Insumos' / 'rutas_archivos_ladm_1_2.json'
