...
class MinisoftOrganizer:
    def __init__(self):
        ...
        self.temp_files_path = self.root_path / "Files" / "Temporary_Files" / "MODELO_LADM_1_2"
        self.reports_path = self.root_path / "Reportes" / "MODELO_LADM_1_2"
    ...
    def clean_reports_directory(self):
        """
        Verifica si existe el directorio MODELO_LADM_1_2 y lo elimina si existe.
        ...
        try:
                    ...
                    for root, dirs, files in os.walk(self.reports_path):
                        for name in files:
                            try:
                                ...
                                error_msg = (
                                    f"No se puede eliminar el directorio MODELO_LADM_1_2 porque\n"
            ...
            # Crear el directorio MODELO_LADM_1_2
            self.reports_path.mkdir(parents=True)
            logger.info(f"Creado directorio: {self.reports_path}")
            return True
            
        except Exception as e:
            error_msg = f"Error al limpiar el directorio MODELO_LADM_1_2: {str(e)}"
            logger.error(error_msg)
            self.show_error_dialog(error_msg)
            return False
    ...
    def copy_gpkg_files(self):
        """Copia archivos .gpkg y la carpeta GPKG_ORIGINAL al directorio INSUMOS"""
        try:
            # Crear directorio INSUMOS si no existe
            insumos_dir = self.reports_path / "INSUMOS"
            insumos_dir.mkdir(parents=True, exist_ok=True)

            # Crear y copiar a GPKG_ETL
            gpkg_etl_dir = insumos_dir / "GPKG_ETL"
            gpkg_etl_dir.mkdir(parents=True, exist_ok=True)

            # Copiar archivos .gpkg a GPKG_ETL
            gpkg_files = list(self.temp_files_path.glob("*.gpkg"))
            if not gpkg_files:
                logger.warning("No se encontraron archivos .gpkg para copiar")
            else:
                for gpkg_file in gpkg_files:
                    shutil.copy2(gpkg_file, gpkg_etl_dir / gpkg_file.name)
                    logger.info(f"Archivo GPKG copiado a GPKG_ETL: {gpkg_file.name}")

            # Copiar carpeta GPKG_ORIGINAL y su contenido
            gpkg_original_source = self.temp_files_path / "GPKG_ORIGINAL"
            gpkg_original_dest = insumos_dir / "GPKG_ORIGINAL"

            if gpkg_original_source.exists():
                if gpkg_original_dest.exists():
                    shutil.rmtree(gpkg_original_dest)
                shutil.copytree(gpkg_original_source, gpkg_original_dest)
                logger.info("Carpeta GPKG_ORIGINAL copiada completamente")
            else:
                logger.warning("Carpeta GPKG_ORIGINAL no encontrada en la fuente")

            return True
        except Exception as e:
            logger.error(f"Error en el proceso de copia de GPKG: {str(e)}")
            return False

    def copy_docx_files(self):
        """Copia archivos .docx a la raíz de Reportes/MODELO_LADM_1_2"""
        try:
