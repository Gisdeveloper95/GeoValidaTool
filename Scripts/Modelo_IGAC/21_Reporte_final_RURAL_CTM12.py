...
class WordAutomation:
    ...
    def get_municipality_info(self, dataset):
        ...
        try:
            ...
            # Para rural, siempre usar R_VEREDA_CTM12
    ...
    def get_conteo_variables(self, dataset, municipio):
        ...
        try:
            ...
            try:
                ...
                try:
                    # Intentar calcular sumando los valores individuales
    ...
    def get_consistencia_variables(self, dataset):
        """Obtiene todas las variables relacionadas con consistencia con manejo mejorado para rural"""
        print(f"\nObteniendo variables de consistencia para {dataset}")
        results = {}
        
        try:
            # Consultas para registro_errores.db adaptadas para rural
            error_queries = {
                'no_huecos': """
            ...
            # Consultas para excepciones adaptadas para rural
            exception_queries = {
                'no_huecos_': """
                    SELECT COUNT(*) 
                    FROM RURAL_CTM12 
            ...
            # Consulta para consistencia_total adaptada para rural
            consistencia_query = """
            SELECT (
                R_UNIDAD_CTM12 + 
                R_TERRENO_CTM12 + 
                R_NOMENCLATURA_VIAL_CTM12 +
                R_TERRENO_CTM12_R_VEREDA_CTM12 + 
                R_UNIDAD_CTM12_R_TERRENO_CTM12 +
                R_NOMEN_DOMICILIARIA_CTM12 + 
                R_UNIDAD_CTM12_R_CONSTRUCCION_CTM12 +
                R_NOMEN_DOMICILIARIA_CTM12_R_TERRENO_CTM12 + 
                R_VEREDA_CTM12 +
                R_CONSTRUCCION_CTM12_R_TERRENO_CTM12 +
                R_CONSTRUCCION_CTM12 + 
                R_VEREDA_CTM12_R_SECTOR_CTM12 +
                R_SECTOR_CTM12
    ...
    def verify_database_integrity(self):
        """Verifica la integridad de las bases de datos para el caso rural"""
        print("\nVerificando integridad de bases de datos...")
        
        integrity_checks = {
            'conteo': [
                "SELECT name FROM sqlite_master WHERE type='table'",
                "SELECT * FROM RURAL_CTM12 LIMIT 1"
            ],
            'errores': [
                "SELECT name FROM sqlite_master WHERE type='table'",
                "SELECT * FROM RURAL_CTM12 LIMIT 1"
    ...
    def generate_diagnostic_report(self):
        ...
        try:
            report_path = self.temp_root / "word_automation_rural_diagnostic.txt"
            ...
            with open(report_path, 'w', encoding='utf-8') as f:
                f.write("=== Reporte de Diagnóstico - Proceso Rural ===\n\n")
    ...
    def get_omision_comision_variables(self, dataset):
        """Obtiene variables de omisión/comisión con manejo mejorado de errores para rural"""
        print(f"\nObteniendo variables de omisión/comisión para {dataset}")
        results = {}
        
        try:
            # Definir las consultas con sus descripciones para rural
            queries = {
                'regis1': {
                    'query': "SELECT SUM(omision_terrenos) AS total_omision_terrenos FROM Rural",
                    'description': "Omisión de Terrenos"
                },
                'regis2': {
                    'query': "SELECT SUM(comision_terrenos) AS total_omision_terrenos FROM Rural",
                    'description': "Comisión de Terrenos"
                },
                'regis3': {
                    'query': "SELECT SUM(omision_unidades) AS total_omision_terrenos FROM Rural",
                    'description': "Omisión de Unidades de Construcción"
                },
                'regis4': {
                    'query': "SELECT SUM(comision_unidades) AS total_omision_terrenos FROM Rural",
                    'description': "Comisión de Unidades de Construcción"
                },
                'regis5': {
                    'query': "SELECT SUM(duplicados) AS total_omision_terrenos FROM Rural",
