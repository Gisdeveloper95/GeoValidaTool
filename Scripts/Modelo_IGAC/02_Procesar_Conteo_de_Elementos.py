...
def obtener_ruta_base():
    ...
    raise Exception(
        ...
        "- Files/Temporary_Files/MODELO_IGAC/db (se creará automáticamente)\n"
        "- Scripts"
    )
...
def crear_ruta_sqlite():
    ...
    ruta_base = obtener_ruta_base()
    sqlite_dir = os.path.join(ruta_base, "Files", "Temporary_Files", "MODELO_IGAC", "db")
...
def procesar_dataset(dataset_path, dataset_name):
...
def analizar_gdb(gdb_path, datasets_permitidos):
    ...
    for ds in datasets:
        ...
        dataset_name, dataset_results = procesar_dataset(dataset_path, ds)
...
def main():
    try:
        ...
        ruta_base = obtener_ruta_base()
        gdb_path = os.path.join(ruta_base, "Files", "Temporary_Files", "MODELO_IGAC")
