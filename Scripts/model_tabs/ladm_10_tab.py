...
from utils.process_manager import ProcessManager
from utils.config_dialog_ladm_10 import ConfigDialogLADM10

class LADM10ModelTab(BaseModelTab):
    ...
    def setup_header(self, parent_layout):
        ...
        config_label = QLabel("Configuración LADM_COL 1.0")  # Cambio de título
    ...
    def show_process_selector(self):
        ...
        # Crear un diálogo personalizado para el modelo LADM 1.0
        dialog = ProcessSelectorDialog(self, self.process_manager)
    
        # Actualizar la lista de procesos para el modelo LADM 1.0
        dialog.processes = [
    ...
    def show_config_dialog(self):
        """Muestra el diálogo de configuración específico para LADM 1.0"""
        dialog = ConfigDialogLADM10(self)
        if dialog.exec():
            self.has_inputs = True
            self.update_button_states()
            self.add_log("Configuración de insumos LADM 1.0 guardada")
    ...
    def check_existing_configuration(self):
        ...
        json_path = os.path.join(self.project_root, "Files", "Temporary_Files", 
                                "Ruta_Insumos", "rutas_archivos_ladm_1_0.json")
    ...
    def execute_all_processes(self):
        """Ejecuta todos los procesos del modelo LADM 1.0"""
        if not self.has_inputs:
            QMessageBox.warning(self, "Error", 
                            "Debe configurar los insumos antes de ejecutar procesos.")
            return
        ...
        modelo_dir = os.path.join(temp_dir, "MODELO_LADM_1_0")
        ...
        if os.path.exists(modelo_dir) and any(os.scandir(modelo_dir)):
            ...
            try:
                ...
                self.add_log("Contenido de MODELO_LADM_1_0 eliminado exitosamente")
            except Exception as e:
                self.add_log(f"Error al limpiar MODELO_LADM_1_0: {str(e)}")
                return

        # Configurar todos los scripts
        try:
            scripts_dir = os.path.join(self.project_root, "Scripts", "MODELO_LADM_1_0")
            scripts = [
    ...
    def run_data_structuring(self):
        """Ejecuta los primeros 5 procesos del modelo LADM 1.0"""
        try:
            if self.process_manager.is_running:
                return False
                    
            # Verificar directorio MODELO_LADM_1_0
            temp_dir = os.path.join(self.project_root, "Files", "Temporary_Files")
            modelo_dir = os.path.join(temp_dir, "MODELO_LADM_1_0")
            ...
            if os.path.exists(modelo_dir) and any(os.scandir(modelo_dir)):
                response = QMessageBox.warning(
                    self,
                    "Archivos existentes",
                    "Se eliminarán los archivos dentro de MODELO_LADM_1_0 para iniciar este proceso. ¿Desea Continuar?",
                    QMessageBox.Yes | QMessageBox.No,
                    QMessageBox.No
                )
