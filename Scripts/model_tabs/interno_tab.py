...
class InternoModelTab(BaseModelTab):
    ...
    def setup_ui(self):
        ...
        # Solo dos botones para el modelo interno
        process_names = [
    ...
    def setup_header(self, parent_layout):
        ...
        config_label = QLabel("Configuración Modelo Interno")
    ...
    def show_process_selector(self):
        ...
        # Crear un diálogo personalizado para el modelo interno
        dialog = ProcessSelectorDialog(self, self.process_manager)
        
        # Actualizar la lista de procesos para el modelo interno
        dialog.processes = [
    ...
    def show_config_dialog(self):
        ...
        from utils.config_dialog_interno import ConfigDialogInterno
        dialog = ConfigDialogInterno(self)
        if dialog.exec():
    ...
    def check_existing_configuration(self):
        ...
        json_path = os.path.join(self.project_root, "Files", "Temporary_Files", 
                                "Ruta_Insumos", "rutas_archivos_interno.json")
    ...
    def execute_all_processes(self):
        """Ejecuta todos los procesos del modelo interno"""
        if not self.has_inputs:
            QMessageBox.warning(self, "Error", 
                            "Debe configurar los insumos antes de ejecutar procesos.")
            return
        ...
        modelo_interno_dir = os.path.join(temp_dir, "MODELO_INTERNO_1_0")
        
        if os.path.exists(modelo_interno_dir) and any(os.scandir(modelo_interno_dir)):
            ...
            try:
                for item in os.listdir(modelo_interno_dir):
                    item_path = os.path.join(modelo_interno_dir, item)
                    if os.path.isfile(item_path):
                        os.remove(item_path)
                    elif os.path.isdir(item_path):
                        shutil.rmtree(item_path)
                self.add_log("Contenido de MODELO_INTERNO_1_0 eliminado exitosamente")
            except Exception as e:
                self.add_log(f"Error al limpiar MODELO_INTERNO_1_0: {str(e)}")
                return

        # Configurar todos los scripts
        try:
            scripts_dir = os.path.join(self.project_root, "Scripts", "MODELO_INTERNO_1_0")
            scripts = [
    ...
    def run_data_structuring(self):
        """Ejecuta los primeros 5 procesos del modelo interno"""
        try:
            if self.process_manager.is_running:
                return False
                    
            # Verificar directorio MODELO_INTERNO_1_0
            temp_dir = os.path.join(self.project_root, "Files", "Temporary_Files")
            modelo_interno_dir = os.path.join(temp_dir, "MODELO_INTERNO_1_0")
            ...
            if os.path.exists(modelo_interno_dir) and any(os.scandir(modelo_interno_dir)):
                response = QMessageBox.warning(
                    self,
                    "Archivos existentes",
                    "Se eliminarán los archivos dentro de MODELO_INTERNO_1_0 para iniciar este proceso. ¿Desea Continuar?",
                    QMessageBox.Yes | QMessageBox.No,
                    QMessageBox.No
                )
            ...
            self.add_log(f"Error al iniciar la estructuración de datos: {str(e)}")
            return False
    
    def run_reports(self):
        """Ejecuta los últimos 5 procesos del modelo interno"""
        try:
