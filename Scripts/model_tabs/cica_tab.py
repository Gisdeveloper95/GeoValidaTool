...
class CICAModelTab(BaseModelTab):
    def __init__(self, parent=None, model_name="", scripts_dir=""):
        ...
        self.container_layout.setContentsMargins(0, 0, 0, 0)  # Eliminar márgenes
    ...
    def setup_ui(self):
        ...
        # Marco inferior para los tres botones de proceso
        ...
        # Configurar botones de proceso dentro del contenedor inferior
        ...
        process_names = [
            ...
            "2. Consitencia Formato",
            "3. Omision-Comision + Reportes"
        ]
    ...
    def setup_header(self, parent_layout):
        ...
        config_label = QLabel("Configuración CICA / CONSERVACION")
    ...
    def show_process_selector(self):
        ...
        if not self.has_inputs or not self.has_zones:
            QMessageBox.warning(self, "Error", 
                            "Debe configurar los insumos y zonas antes de ejecutar procesos.")
            return
    ...
    def show_config_dialog(self):
        ...
        if dialog.exec():
            ...
            self.add_log("Configuración de insumos guardada")
    ...
    def check_existing_configuration(self):
        ...
        json_path = os.path.join(self.project_root, "Files", "Temporary_Files", 
                                "Ruta_Insumos", "rutas_archivos.json")
    ...
    def execute_all_processes(self):
        ...
        if os.path.exists(modelo_igac_dir) and any(os.scandir(modelo_igac_dir)):
            response = QMessageBox.warning(
                self,
                "Archivos existentes",
                "Se eliminarán archivos anteriores para iniciar este proceso. ¿Desea Continuar?",
            ...
            try:
                ...
                self.add_log("Contenido de MODELO_IGAC eliminado exitosamente")
            except Exception as e:
                self.add_log(f"Error al limpiar MODELO_IGAC: {str(e)}")
                return
        
        batch_processor = BatchProcessor(self.process_manager)
    ...
    def run_data_structuring(self):
        ...
        try:
            ...
            # Verificar directorio MODELO_IGAC (no MODELO_INTERNO_1_0)
            temp_dir = os.path.join(self.project_root, "Files", "Temporary_Files")
            modelo_dir = os.path.join(temp_dir, "MODELO_IGAC")  # Cambiado
            ...
            # Usar los scripts correctos de CICA
            scripts = [
            ...
            scripts_dir = os.path.join(self.project_root, "Scripts", "Modelo_IGAC")
    ...
    def setup_status_and_console(self):
        ...
        # Lista de descripciones de procesos con sus estilos
        process_descriptions = [
            ("Copiar Archivos necesarios", False),
            ("Procesar Conteo de Elementos", False),
            ("Crear Topologías", False),
            ("Aplicar Reglas Topologicas", False),
            ("Exportar Erro.Topológicos a SHP 1/2", False),
            ("Exportar Erro.Topológicos a SHP 2/2", True),
            ("Generar DB registro Errores", False),
            ("Exportar Err.Topologicos segun reglas a SHP", False),
            ("Diligenciar Err. y Excep. a Excel", False),
            ("Encabecado Formato Consitencia Logica", False),
            ("Toolbox Consistencia Formato", False),
            ("Toolbox Intersecciones Consistencia Formato", False),
            ("Generar SHP Consistencia Formato", False),
            ("Generar DB registro Errores Consistencia", False),
            ("Diligenciar Err. Consitencia a Excel", True),
            ("Generar DB registro Excepciones Consistencia", False),
            ("Diligenciar Excepciones Consitencia a Excel", False),
            ("Toolbox Omision-Comision", False),
            ("Conteo Poligonos Duplicados", False),
            ("Análisis Omisión Comisión", False),
